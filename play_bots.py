...
def main():
    ...
    print("Mode: AI vs AI (self-play)")

    renderer = Renderer()
    game = GomokuGame()

    # Wait for a click to start
    renderer.render(game, status="Click anywhere to start AI self-play")
    print("Click anywhere in the window to begin.")
    waiting = True
    while waiting:
    ...
    while not game.is_terminal():
        # Check for window close at any time
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                renderer.close()
                return
        ...
        player_name = "Black (X)" if game.current_player == 1 else "White (O)"
        renderer.render(game, status=f"{player_name} thinking... ({NUM_SIMULATIONS} sims)")
        pygame.event.pump()
        ...
        print(f"{player_name} plays: ({r},{c})  [top: {top_str}]")
